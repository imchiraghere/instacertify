      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-[24px] lg:grid-cols-6">
        {LOGO_ITEMS.map((logo, index) => (
          <Link
            key={index}
            href="/"
            className={`rounded-lg transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px] bg-white h-[80px] flex items-center justify-center ${
              logo.extraClass || ""
            }`}
          >
            <img
